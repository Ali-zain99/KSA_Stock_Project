...
db = SQLAlchemy(app)
import os
UPLOAD_FOLDER = os.path.join(os.getcwd(), 'KSA_Stock_Project/static/images')
...
def chatbot():
    ...
    try:
        ...
        if 'visualize' in query.lower():
            stock_1_price = [10, 20, 30, 40, 50]  # Use square brackets for lists
            stock_2_price = [20, 30, 40, 50, 60]  # Use square brackets for lists

            # Plot the data
            plt.plot(stock_1_price, label='Stock 1')
            plt.plot(stock_2_price, label='Stock 2')
            ...
            # Show the plot
            image_path = os.path.join(app.config['UPLOAD_FOLDER'], 'stock_prices.png')
            plt.savefig(image_path)
            plt.close()

            # Return the image URL
            image_url = f"/stock_prices.png"
            return jsonify({"response": "Here is the visualization:", "image_url": image_url})
